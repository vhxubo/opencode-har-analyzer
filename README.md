...
当前版本已覆盖用量聚合、分钟级异常检测与测活标注。面向**中转站运营分析**，以下方向暂未实现，留作后续扩展（按性价比排序）：

| 方向 | 关注点 | 可用信号 |
|---|---|---|
| 明细/服务端对账 | 逐次求和 vs 服务端 daily 汇总的成本差异，差值即漏记/错记信号 | `records` 与 `daily` 双口径 |
| 单价反推 | 同一模型 token 单价波动，识别异常路由/计费 | `cost ÷ (input+output)` |
| 缓存命中率 | 缓存利用差的 key/模型成本虚高 | `cacheReadTokens / inputTokens` |
| 推理 token 占比 | 推理单独计费，占比异常高需关注 | `reasoningTokens` |
| Key 活跃度分层 | 活跃 / 沉默 / 试用后弃用 | 首末次调用间隔 |
| 成本集中度 | 少数 key 消耗大部分成本（帕累托） | 按 key 成本分布 |
| 单 key 并发度 | 同 key 同时跨多模型调用（脚本并行） | 同分钟多模型 |
| 周期性信号 | 定时任务/批处理调用（cron 类） | 按小时/星期自相关 |
| 重复调用指纹 | 重试风暴 / 死循环 | 同 key+模型+token 重复 |
| 会话特征 | 交互式 vs 批处理 | 相邻调用间隔聚类 |
| 异常增强 | 全缓存调用、零成本调用、超长输出 | token/成本阈值 |
