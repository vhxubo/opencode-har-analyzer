...
| 异常分析 | 分钟级调用次数检测，短时高频自动标记，可调参数、可展开明细 |
...
- **参数**（卡片右上角可调）：合并间隔（分钟）、最少次数（次/分）、倍率（×）；
- **展示**：
  - 摘要卡：异常事件数、涉及 key、异常调用合计、最高频率（次/分）；
  - 异常表格：时间窗口（含持续时长）、key、调用次数、频率、模型分布、占该 key 总量 %；**点击行展开**窗口内调用明细；
  - 异常时间线图表：每事件调用次数柱状，按 key 着色；
- **联动**：遵循全局 key 与时间区间筛选；无异常 / 未选 key 分别提示。

### 7. 调用明细表

- 列：时间、Key（displayName + 完整 keyID）、模型、Provider、输入/输出/推理/缓存 token、成本、Plan、记录 ID；
